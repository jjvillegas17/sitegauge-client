...
class DeleteUser extends Component {
    ...
    getUsers = async () => {
        const response = await axios.get(`https://sitegauge.io/api/unblocked-users`); 
    ...
    delete = (e) => {
        e.preventDefault();
        axios.post(`https://sitegauge.io/api/admin/block`, {users: this.state.usersToDelete},
            {
                                    ...
                                    </tbody>
                                    {
                                        ...
                                        <tfoot className="full-width">
                                            <tr>
                                              <td colSpan="3">
                                                <Modal
                                                    trigger={<button className="negative ui button" onClick={this.handleOpen}>Block Users</button>}
                                                    open={this.state.modalOpen}   
                                                    onClose={this.handleClose}
                                                    basic
                                                    size='small'
                                                  >
                                                    <Header icon='archive' content='Block these users?' />
                                                    <Modal.Content>
                                                      <h3>Are you sure you want to block the selected users?</h3>
                                                    </Modal.Content>
                                                    <Modal.Actions>
